 ...
 * @author Samuele
 */
public class TestContacts implements Test {
    ...
    private final String EMAIL_VERIFICATION = "[a-zA-Z0-9]+[._a-zA-Z0-9!#$%&'*+-/=?^_`{|}~]*[a-zA-Z]*@[a-zA-Z0-9-\\-]{2,30}\\.[a-zA-Z.]{2,20}";
    ...
    public TestContacts(Site site, String id) {
        current = site;
    ...
    public TestContacts(Site url, WebDriver webDriv, String id) {
        current = url;
        wb = webDriv;
        listEmail = new ArrayList<>();
        timestamp = new Timestamp(System.currentTimeMillis());
    ...
    public Document toDocument() {
        return new Document("name", getName()).append("timestamp", timestamp.toString()).append("task_id", id).append("info", info).append("result", getResult()).append("details", listEmail);
    }
    ...
    public static Test fromDocument(Document d, Site s) {
        ...
        TestContacts test = new TestContacts(s, null, d.getString("task_id"));
